...
from .models import AutoTest, ProjectConfig, ObjectConfig
...
@admin.register(AutoTest)
class AutoTestAdmin(admin.ModelAdmin):
    list_display = ('project_name','class_name','name','is_run','schedule','result','pre_data_file','script_path')
    ...
    def make_story_public(self, request, queryset):
        ...
        # clear_chart_info()
        full_path = request.path.split('/')
        run_test(queryset,"service",{"app_name":full_path[2],"model_name":full_path[3]})
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        :return:
        """
        self.exclude = ('result','schedule','creater')
        form = super(AutoTestAdmin,self).get_form(request,obj,**kwargs)
        return form
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            ...
            if current_pre_path.find('.') > -1:
                remove_file_common(current_pre_path)
            # write_path = current_path
        AutoTest._meta.get_field("script_path").upload_to = upload_to_path
        AutoTest._meta.get_field("pre_data_file").upload_to = pre_data_path

...
class TestApi(unittest.TestCase):
    ...
    def tearDownClass(self):
        write_excel(self.test_result,self.test_run_id,"api")
        # write_log("接口执行成功")
        print "end testing"

    @ddt.data(*get_total_data_v2("api",True))
    def test_Run(self,datas):
        ...
        if not datas.has_key('request_title'):
            ...
            cookie = request_infos.get('cookie')
            url = request_infos.get('url')
            request_type = request_infos.get('request_type')
            ...
            try:
                # 4、调用接口得到返回值
                response = api_response(request_type,url,parameters,cookie)
                # 5、进行检查check
                for key in assert_data.keys():
                    ...
                    if assert_key == "other":
                        ...
                        other_assert_key = other_assert_data.get("key")
                        value_list = other_assert_data.get("value")
                        for i in range(len(other_assert_key)):
                            ...
                            if other_key in configSetting.ASSERT_OTHER_KEY:
                                ...
                                elif other_key in configSetting.ASSERT_VALUE:
                                    list_key = []
                                    other_value_key = value_list[i].keys()
                                    assert_value=value_list[i].get(other_value_key[0])
                ...
                self.test_result.append({'project': request_title.get('project'),
                                         ...
                                         'url': url, 'cookie': cookie,'request_type': request_type})
                # print "检查通过"
